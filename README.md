...
Embedded firmware template for the Texas Instruments TM4C123GH6PM / EK-TM4C123GXL LaunchPad using:
...
This repository is a working Cortex-M4F firmware project with:
...
- A FreeRTOS-based demo application with tasks, queues, and UART interrupts
...
Open a serial terminal at 115200 8-N-1 on the LaunchPad's virtual COM port to see output.
...
- Configures UART0 on PA0/PA1 at 115200 8-N-1.
...
Most user code lives under `src/`. You generally don't need to modify CMSIS, FreeRTOS, or TivaWare directly.
...
- `cmake ..` configures the project:
  ...
  - CPU/FPU flags for Cortex-M4F (`-mcpu=cortex-m4 -mthumb -mfloat-abi=hard -mfpu=fpv4-sp-d16`)
- `cmake --build . -j4` builds with 4 parallel jobs (tune `-j` for your CPU).

On success, `build/` contains:

- `tm4c123gxl.elf` - ELF image with symbols and debug info.
- `tm4c123gxl.bin` - raw binary.
- `tm4c123gxl.hex` - Intel HEX.
- `output.map` - linker map (memory layout, symbol addresses).
...
Note: `newlib-nano` may print warnings like `_write is not implemented and will always fail`; these are normal for bare-metal unless you hook up low-level syscalls.

## Flashing with OpenOCD

### 1. Connect the EK-TM4C123GXL

Plug the EK-TM4C123GXL LaunchPad into your Windows machine via USB.

Ensure the on-board ICDI debugger is recognized (Windows installs drivers and exposes a debug interface + virtual COM port).

WSL uses the same USB devices via Windows; OpenOCD will communicate through those drivers.

### 2. One-line flash command (no GDB)
...
- Use the built-in EK-TM4C123GXL board config.
...
If you want breakpoints and single-stepping with FreeRTOS tasks:
...
- CPU / FPU flags for Cortex-M4F (ARM_CM4F port).
- `heap_4.c` allocator.
...
- Diagnostics:
  ...
  - `uxTaskGetStackHighWaterMark()` for per-task stack usage.
  - `vTaskList()` and `vTaskGetRunTimeStats()` if `configUSE_TRACE_FACILITY` and `configGENERATE_RUN_TIME_STATS` are set.

You can adjust these options in `src/FreeRTOSConfig.h` to match your use case (debug heavy vs production-minimal).
...
- Enable and use more FreeRTOS run-time stats for profiling.

The current structure already demonstrates:

- Interrupt-driven UART feeding a FreeRTOS queue from ISRs.
...
This project bundles third-party components:
